...
const buyPrice = process.argv[3]
const sellPrice = process.argv[4]
...
var loop = function(account_name) {
	async.parallel(({
		...
		if (price <= buyPrice && hodl == 0) {
			buy(account_name, price, myeos)
		}
...
var buy = function(account, price, amount) {
	shell.exec(unlock_wallet_cmd)
	logger.info("准备买入!");
	var buyMsg = `价格浮动至 ${price} EOS/智子, 买入成功! `;
	var title = account;

	var buyCmd = `~/eos/build/programs/cleos/cleos -u ${config.httpEndpoint} transfer ${account} itedeathstar "${amount}" "buy"`;
	var result = shell.exec(buyCmd, {
		async: true
	});
	if (result.code === 0) {
